 ...
 *
 * <p>All class constant pool entries get direct references to the corresponding classes. These
 * references make it more convenient to travel up and across the class hierarchy.
 *
 * <p>All field and method reference constant pool entries get direct references to the
 * corresponding classes, fields, and methods.
 *
 * <p>All name and type constant pool entries get a list of direct references to the classes listed
 * in the type.
 *
 * <p>The classpools are searched by keys and not by Clazz.getName(), so after obfuscation make sure
...
public class ClassReferenceInitializer
  ...
  /**
   * Returns the single class referenced by the given descriptor, or <code>null</code> if there
  ...
  private class KotlinDefaultMethodInitializer implements KotlinFunctionVisitor {
     ...
     * function.
     *
     * <p>The descriptor matches the original method but with 2 or more extra parameters:
     *
     * <p>- the first parameter is an instance reference if the original method was not static. -
