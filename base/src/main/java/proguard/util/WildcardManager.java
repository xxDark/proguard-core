 ...
 *
 * <p>Each instance represents a scope in which wildcards can be specified and referenced.
 *
...
public class WildcardManager {
   ...
   * @param nextMatcher an optional next matcher for the remainder of the string.
   */
  public VariableStringMatcher createVariableStringMatcher(
    ...
    VariableStringMatcher variableStringMatcher =
        new VariableStringMatcher(
            allowedCharacters, disallowedCharacters, minimumLength, maximumLength, nextMatcher);
   ...
   *
   * @param index the 0-based index of previously created and remembered VariableStringMatcher
   *     instances.
   * @param nextMatcher an optional next matcher for the remainder of the string.
   */
  public MatchedStringMatcher createMatchedStringMatcher(int index, StringMatcher nextMatcher) {
    return new MatchedStringMatcher(variableStringMatchers.get(index), nextMatcher);
  }
  ...
  public StringFunction createMatchedStringFunction(String expression) {
        ...
        StringFunction additionalStringFunction =
            prefixIndex == index
                ...
                : new ConcatenatingStringFunction(
                    new ConstantStringFunction(expression.substring(prefixIndex, index)),
                    matchedStringFunction);
    ...
    return stringFunction == null
        ? new ConstantStringFunction(expression)
        : prefixIndex == expression.length()
            ? stringFunction
            : new ConcatenatingStringFunction(
                stringFunction, new ConstantStringFunction(expression.substring(prefixIndex)));
  }
  ...
  public int wildCardIndex(String regularExpression, int index) throws IllegalArgumentException {
    if (variableStringMatchers == null || regularExpression.charAt(index) != '<') {
      return -1;
    }
    ...
    if (closingBracketIndex < 0) {
      throw new IllegalArgumentException(
          "Missing closing angular bracket after opening bracket at index "
              + index
              + " in ["
              + regularExpression
              + "]");
