 ...
 *
 * <p>Multiple entries and output streams can be open at the same time. The entries are added to the
 * central directory in the order in which they are opened, but the corresponding data are only
 * written when their output streams are closed.
 *
 * <p>The code automatically computes the CRC and lengths of the data, for compressed and
 * uncompressed data.
 *
...
public class ZipOutput {
   ...
   */
  public ZipOutput(
      OutputStream outputStream, int uncompressedAlignment, boolean useZip64, String comment) {
   ...
   */
  public ZipOutput(
      OutputStream outputStream, byte[] header, int uncompressedAlignment, boolean useZip64)
      throws IOException {
  ...
  /**
   * Creates a new zip entry, returning an output stream to write its data. It is the caller's
   * responsibility to close the output stream.
   *
   * @param name the name of the zip entry.
   * @param compress specifies whether the entry should be compressed with the default alignment.
   * @param modificationTime the modification date and time of the zip entry, in DOS format.
   * @return an output stream for writing the data of the zip entry.
   */
  public OutputStream createOutputStream(String name, boolean compress, int modificationTime)
      throws IOException {
    return createOutputStream(name, compress, uncompressedAlignment, modificationTime, null, null);
  }
  ...
  public OutputStream createOutputStream(
      String name, boolean compress, int uncompressedAlignment, int modificationTime)
      throws IOException {
    return createOutputStream(name, compress, uncompressedAlignment, modificationTime, null, null);
  }
   ...
   * @param uncompressedAlignment the requested alignment of uncompressed data.
   * @param modificationTime the modification date and time of the zip entry, in DOS format.
   * @param extraField optional extra field data. These should contain chunks, each with a short ID,
   *     a short length (little endian), and their corresponding data. The IDs 0-31 are reserved for
   *     Pkware. Java's jar tool just specifies an ID 0xcafe on its first entry.
  ...
  public OutputStream createOutputStream(
    ...
    ZipEntry entry =
        new ZipEntry(name, compress, uncompressedAlignment, modificationTime, extraField, comment);
  ...
  protected long writeEntriesOfCentralDirectory() throws IOException {
    if (DEBUG) {
      System.out.println(
          "ZipOutput.writeEntriesOfCentralDirectory (" + zipEntries.size() + " entries)");
    }
   ...
   * @param centralDirectorySize the size of the central directory, not counting the empty header
   *     and the trailer.
   */
  protected void writeEndOfCentralDirectory(long centralDirectoryOffset, long centralDirectorySize)
    ...
    if (DEBUG) {
      System.out.println(
          "ZipOutput.writeEndOfCentralDirectory (" + zipEntries.size() + " entries)");
    }
    ...
    writeInt(
        useZip64
            ? ZIP64_FIELD_TOO_SMALL_32BIT
            : centralDirectoryOffset); // Offset of central directory.
   ...
   *
   * @param centralDirectoryOffset The offset from the start of the archive to the first central
   *     directory record.
   ...
   */
  protected void writeZip64EndOfCentralDirectory(
      long centralDirectoryOffset, long centralDirectorySize) throws IOException {
    if (DEBUG) {
  ...
  /**
   * This class represents a zip entry in its enclosing zip file. It can provide an output stream
   * and write its headers and its data to the main zip output stream. In fact, it automatically
   * writes its local header and data when the output stream is closed.
   */
  private class ZipEntry {
    ...
    /**
     * Creates a new zip entry, returning output stream to write its data. It is the caller's
     * responsibility to close the output stream.
     *
     * @param name the name of the zip entry.
     * @param compressed specifies whether the entry should be compressed.
     * @param uncompressedAlignment the requested alignment of uncompressed data.
     * @param modificationTime the modification date and time of the zip entry, in DOS format.
     * @param extraField optional extra field data. These should contain chunks, each with a short
     *     ID, a short length (little endian), and their corresponding data. The IDs 0-31 are
     *     reserved for Pkware. Java's jar tool just specifies an ID 0xcafe on its first entry.
     * @param comment an optional comment.
    ...
    /** Writes the local file header, which precedes the data, to the main zip output stream. */
    private void writeLocalFileHeader() throws IOException {
      ...
      boolean useZip64ExtraField =
          useZip64 && ((compressedSize >> 32 > 0) || (uncompressedSize >> 32 > 0));
      ...
      int extraFieldLength =
          (useZip64ExtraField
                  ? ZIP64_EXTENDED_SMALL_EXTRA_INFORMATION_FIELD_SIZE
                      + ZIP64_EXTENDED_EXTRA_INFORMATION_FIELD_HEADER_SIZE
                  : 0)
    ...
    /** Writes the file header for the central directory to the main zip output stream. */
    public void writeCentralDirectoryFileHeader() throws IOException {
      ...
      boolean useZip64ExtraField =
          useZip64
              && ((compressedSize >> 32 > 0) || (uncompressedSize >> 32 > 0) || (offset >> 32 > 0));
      ...
      byte[] nameBytes = StringUtil.getModifiedUtf8Bytes(name);
      byte[] commentBytes = comment == null ? null : StringUtil.getModifiedUtf8Bytes(comment);

      writeShort(nameBytes.length);

      int extraFieldLength =
          (useZip64ExtraField
                  ? ZIP64_EXTENDED_LARGE_EXTRA_INFORMATION_FIELD_SIZE
                      + ZIP64_EXTENDED_EXTRA_INFORMATION_FIELD_HEADER_SIZE
                  : 0)
              + (extraField != null ? extraField.length : 0);
    ...
    /**
     * This OutputStream writes its uncompressed zip entry out to its zip output stream when it is
     * closed.
     */
    ...
    /**
     * This OutputStream writes its compressed zip entry out to its zip output stream when it is
     * closed.
     */
    private class CompressedZipEntryOutputStream extends DeflaterOutputStream {
      ...
      public void close() throws IOException {
        ...
        ByteArrayOutputStream byteArrayOutputStream = (ByteArrayOutputStream) super.out;
  ...
  public static void main(String[] args) {
    try {
      ...
      PrintWriter printWriter1 =
          new PrintWriter(
              output.createOutputStream(
                  ...
                  1,
                  0,
                  new byte[] {0x34, 0x12, 4, 0, 0x48, 0x65, 0x6c, 0x6c, 0x6f},
                  "Comment"));
      ...
      PrintWriter printWriter2 =
          new PrintWriter(
              output.createOutputStream("file2.txt", true, 1, 0, null, "Another comment"));
      ...
      PrintWriter printWriter3 =
          new PrintWriter(
              output.createOutputStream("file3.txt", false, 1, 0, null, "Last comment"));
