...
public class JvmInvokeTaintSink extends JvmTaintSink {
  ...
  public static class Builder {
    ...
    public JvmInvokeTaintSink build() {
      return new JvmInvokeTaintSink(
          signature, isValidForSource, takesInstance, takesArgs, takesGlobals, callMatcher);
    }
        ...
        i ->
            result.add(
                new JvmStackLocation(
                    parameterSize - ClassUtil.internalMethodVariableIndex(descriptor, true, i))));
  ...
  public String toString() {
    ...
    if (!takesArgs.isEmpty()) {
      result
          .append(", takes args (")
          .append(
              takesArgs.stream().map(Objects::toString).sorted().collect(Collectors.joining(", ")))
          .append(")");
    }
    if (!takesGlobals.isEmpty()) {
      result
          .append(", takes globals (")
          .append(takesGlobals.stream().sorted().collect(Collectors.joining(", ")))
          .append(")");
    }
